...
import type { Block, BlockType, Comment, Connection, ConnectionType, DirectionHint } from '../types/diagram'
...
function parseDirection(directiveLine: string): DirectionHint {
  ...
  return (['TD', 'LR', 'TB', 'BT', 'RL'] as DirectionHint[]).includes(d) ? d : 'TD'
}
...
export function parseMmd(text: string): ParseResult {
  ...
  if (metaStartIdx !== -1 && metaEndIdx !== -1 && metaEndIdx > metaStartIdx) {
    const metaSection = text.slice(metaStartIdx, metaEndIdx + '%% MMD_META_END'.length)
    ...
    if (jsonLine) {
      ...
      try {
        ...
        if (parsed.version !== '1') {
          ...
          warnings.push('Metadata could not be read. Comments and data fields were not loaded.')
        } else {
          metadata = parsed
          metadataVersion = '1'
        }
  ...
  if (!directiveLine) {
    ...
    return { ok: false, readOnly: true, mmd: text, reason: 'Diagram type is not supported for editing.' }
  }
  ...
  const rawNodes = new Map<string, { nodeType: RawNodeType; label: string; index: number }>()
  const rawEdges: Array<{ sourceId: string; targetId: string; type: ConnectionType }> = []
  let nodeIndex = 0
  ...
  for (const line of lines) {
    ...
    if (line.includes('-->')) {
      if (RE_EDGE_YES.test(line)) {
        ...
        if (m) rawEdges.push({ sourceId: m[1], targetId: m[2], type: 'default' })
      }
      continue
    }
    ...
    if (pillMatch) {
      rawNodes.set(pillMatch[1], { nodeType: 'pill', label: unescapeLabel(pillMatch[2]), index: nodeIndex++ })
      continue
    }

    const resultMatch = line.match(RE_RESULT)
    if (resultMatch) {
      rawNodes.set(resultMatch[1], { nodeType: 'result', label: unescapeLabel(resultMatch[2]), index: nodeIndex++ })
      continue
    }

    const decisionMatch = line.match(RE_DECISION)
    if (decisionMatch) {
      rawNodes.set(decisionMatch[1], { nodeType: 'decision', label: unescapeLabel(decisionMatch[2]), index: nodeIndex++ })
      continue
    }

    const actionMatch = line.match(RE_ACTION)
    if (actionMatch) {
      rawNodes.set(actionMatch[1], { nodeType: 'action', label: unescapeLabel(actionMatch[2]), index: nodeIndex++ })
      continue
    }
  }
  ...
  return { ok: true, blocks, connections, directionHint, metadataVersion, warnings }
}
