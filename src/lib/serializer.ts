...
import type { Block, BlockType, Connection, DiagramFile } from '../types/diagram'
...
function sortBlocks(blocks: Map<string, Block>): Block[] {
  ...
  return [...groups.start, ...groups.action, ...groups.decision, ...groups.result, ...groups.end]
}
...
export function serializeDiagram(diagram: DiagramFile): SerializeResult {
  ...
  if (startBlocks.length > 1) {
    return { ok: false, error: 'Diagram must have exactly one Start block (found multiple).' }
  }
  ...
  for (const conn of connections.values()) {
    ...
    if (!blocks.has(conn.sourceId) || !blocks.has(conn.targetId)) {
      console.warn(`[serializer] Dropping orphaned connection ${conn.id}: ${conn.sourceId} → ${conn.targetId}`)
      continue
    }

    // Start block can't be targetId
    if (conn.targetId === startBlock.id) {
      console.warn(`[serializer] Dropping connection ${conn.id}: start block cannot be a target`)
      continue
    }

    // End block can't be sourceId
    if (endBlockIds.has(conn.sourceId)) {
      console.warn(`[serializer] Dropping connection ${conn.id}: end block cannot be a source`)
      continue
    }
    ...
    if (conn.type === 'yes') {
      if (seenDecisionYes.get(conn.sourceId)) {
        console.warn(`[serializer] Dropping duplicate 'yes' connection from ${conn.sourceId}`)
        continue
      }
    ...
    if (conn.type === 'no') {
      if (seenDecisionNo.get(conn.sourceId)) {
        console.warn(`[serializer] Dropping duplicate 'no' connection from ${conn.sourceId}`)
        continue
      }
  ...
  for (const block of blocks.values()) {
    metaBlocks[block.id] = {
      ...
      comments: block.comments.map((c) => ({ id: c.id, text: c.text, timestamp: c.timestamp })),
    }
  }
  ...
  const metadata: MetadataV1 = { version: '1', meta: metaBlocks, connections: metaConns }
